...
module.exports = (env, argv) => {
  ...
  return {
    entry: './src-modern/scss/livesite.css.scss',
    output: {
      path: path.resolve(__dirname, 'dist-modern'),
      filename: 'livesite.modern.css.js', // Dummy JS file, we only need CSS
      clean: false, // Don't clean, we're building alongside main config
      publicPath: '/dist-modern/', // Ensures font URLs resolve correctly relative to CSS file
    },
    module: {
      rules: [
        {
        ...
        },
        {
          ...
          generator: {
            ...
            filename: (pathData) => {
              ...
              // Extract relative path from src-modern/scss
              const scssPath = path.resolve(__dirname, 'src-modern/scss');
    ...
    plugins: [
      new MiniCssExtractPlugin({
        filename: isProduction ? 'livesite.modern.min.css' : 'livesite.modern.css',
      }),
    ],
    optimization: {
    ...
    resolve: {
      extensions: ['.scss', '.css'],
      modules: [path.resolve(__dirname, 'src-modern/scss'), 'node_modules'],
    },
  };
};
