...
module.exports = (env, argv) => {
  ...
  return {
    entry: './src-classic/scss/livesite_mobile.scss',
    output: {
      path: path.resolve(__dirname, 'dist-classic'),
      filename: 'livesite.mobile.js', // Dummy JS file, we only need CSS
        ...
        },
        {
          ...
          generator: {
            ...
            filename: (pathData) => {
              ...
              // Extract relative path from src-classic/scss
              const scssPath = path.resolve(__dirname, 'src-classic/scss');
    ...
    plugins: [
      new MiniCssExtractPlugin({
        filename: isProduction ? 'livesite.mobile.min.css' : 'livesite.mobile.css',
      }),
    ],
    optimization: {
    ...
    resolve: {
      extensions: ['.scss', '.css'],
      modules: [path.resolve(__dirname, 'src-classic/scss'), 'node_modules'],
    },
  };
};
